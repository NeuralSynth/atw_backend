...
from billing.models import Contract, Invoice, SystemSettings
from ems.models import EMSReport
from patients.models import Patient
from trips.models import ChatMessage, Trip
...
class Command(BaseCommand):
    ...
    def create_users(self):
        ...
        users = {"admin": [], "drivers": [], "paramedics": [], "corporate": [], "vendor": []}
    ...
    def create_patients(self, companies):
        ...
        for i, (name, mrn, dob) in enumerate(patient_data):
            ...
            patients.append(Patient.objects.create(name=name, medical_record_number=mrn, dob=dob, company=company))
    ...
    def create_ems_reports(self, trips):
        ...
        for i, trip in enumerate(completed_trips):
            reports.append(
                EMSReport.objects.create(trip=trip, medical_data=medical_data_templates[i % len(medical_data_templates)])
            )
    ...
    def create_system_settings(self):
        ...
        for key, value, description in settings_data:
            settings.append(
                SystemSettings.objects.create(setting_key=key, setting_value=value, description=description, is_active=True)
            )
